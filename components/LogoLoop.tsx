...
/** Slightly higher tau = softer / slower easing when crossing into hover marquee speed */
...
const useResizeObserver = (
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- mirror upstream hook contract
  }, dependencies);
};
...
export const LogoLoop = React.memo<LogoLoopProps>(
  ({
    ...
    const [copyCount, setCopyCount] =
      useState<number>(ANIMATION_CONFIG.MIN_COPIES);
        ...
        const content = isNodeItem ? (
          <span
            ...
            aria-hidden={
              ...
              !(item as unknown as { ariaLabel?: string }).ariaLabel
            }
          >
    ...
    const logoLists = useMemo(
      () =>
        Array.from({ length: copyCount }, (_, copyIndex) => (
          ...
          >
            {logos.map((logoItem, itemIndex) =>
              renderLogoItem(logoItem, `${copyIndex}-${itemIndex}`),
            )}
          </ul>
        )),
